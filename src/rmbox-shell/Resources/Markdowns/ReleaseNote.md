...
- **注意：** 现在，使用相对路径作为参数的操作在路径解析时会使用 `Environment.CurrentDirectory` 进行基路径的解析。该路径通常为 Rmbox 启动时所在的文件夹路径（不一定是 Rmbox 本体所在的文件夹）。
